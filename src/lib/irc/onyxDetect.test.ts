...
describe('isOnyxMyinfo', () => {
  ...
  it('matches NETWORK=Onyx and residual NETWORK=IRCXNet', () => {
    expect(isOnyxMyinfo('NETWORK=Onyx CHANTYPES=#& are supported by this server')).toBe(true);
    expect(isOnyxMyinfo('NETWORK=IRCXNet CHANTYPES=#& are supported by this server')).toBe(true);
  });
