...
export default function Header() {
  ...
  return (
    <header class="flex items-center gap-1.5 sm:gap-3 px-1.5 sm:px-4 h-11 sm:h-12 border-b border-white/[0.05] bg-gray-950/80 backdrop-blur-sm shrink-0 relative">
      {/* Mobile menu */}
      <button
        onClick={() => toggleSidebar()}
        class="lg:hidden w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-gray-100 active:bg-white/[0.06] transition-colors"
        aria-label="Menu"
      >
        <svg class="w-[20px] h-[20px] sm:w-[18px] sm:h-[18px]" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
          <path d="M2 4h12M2 8h8M2 12h10" />
        </svg>
      </button>

      {/* Buffer name + topic */}
      <div ref={(el) => (topicRef = el)} class="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-baseline sm:gap-3 justify-center relative">
        <div class="flex items-center gap-1.5">
          <h2 class="text-[14px] sm:text-[15px] font-bold tracking-tight text-gray-100 truncate leading-tight">{bufName()}</h2>
          <Show when={!isIrc()}>
            <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-white/[0.06] text-gray-500 border border-white/[0.06] shrink-0">
        ...
        <Show when={title()}>
          <button
            ...
            class="text-[11px] text-gray-600 sm:text-gray-500 truncate leading-tight sm:text-[11px] max-w-full text-left hover:text-gray-300 transition-colors"
      ...
      <div class="flex items-center gap-0.5 sm:gap-2 shrink-0">
        ...
        <Show when={canCall()}>
          <button
            ...
            class="w-9 h-9 sm:w-7 sm:h-7 flex items-center justify-center rounded-full text-gray-400 hover:text-[var(--custom-accent,#818cf8)] hover:bg-white/[0.04] active:bg-white/[0.06] transition-all"
            title={isPrivate() ? 'Voice call' : 'Join voice'}
            aria-label={isPrivate() ? 'Voice call' : 'Join voice'}
          >
            <svg class="w-[14px] h-[14px] sm:w-[12px] sm:h-[12px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          ...
          <button
            ...
            class="w-9 h-9 sm:w-7 sm:h-7 flex items-center justify-center rounded-full text-gray-400 hover:text-emerald-400 hover:bg-white/[0.04] active:bg-white/[0.06] transition-all"
        ...
        >
          <svg class="w-[14px] h-[14px] sm:w-[12px] sm:h-[12px]" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
        ...
        <Show when={isChannel()}>
          <button
            ...
            class="flex items-center gap-1 h-9 sm:h-7 px-2.5 sm:px-2.5 rounded-full text-[12px] sm:text-[11px] font-medium transition-all"
            classList={{
          ...
          >
            <svg class="w-[15px] h-[15px] sm:w-[12px] sm:h-[12px]" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
              <circle cx="6" cy="5" r="2.5" />
              <path d="M1 14c0-3 2-5.5 5-5.5s5 2.5 5 5.5" />
            </svg>
            <Show when={nickCount() > 0}>
              <span class="tabular-nums">{nickCount()}</span>
            </Show>
          </button>
        </Show>
      </div>
