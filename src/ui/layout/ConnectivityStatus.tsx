  ...
  return (
    <Show when={visible()}>
      <div
        ...
        role="status"
        aria-live="polite"
        class="fixed left-1/2 top-[max(0.75rem,env(safe-area-inset-top))] z-[70] flex w-[min(calc(100%_-_1.5rem),680px)] -translate-x-1/2 items-center gap-3 rounded-2xl border border-[var(--color-gray-100)]/15 bg-[var(--color-gray-950)]/95 px-3.5 py-3 text-[var(--color-gray-100)] shadow-2xl shadow-black/45 backdrop-blur-xl sm:px-4"
      >
        <span
          class={`h-2.5 w-2.5 shrink-0 rounded-full ${browserOnline() ? 'bg-amber-400' : 'bg-[var(--role-mention,#f87171)]'}`}
          aria-hidden="true"
        />
        <div class="min-w-0 flex-1">
          ...
          <p class="mt-0.5 text-[11px] leading-relaxed text-[var(--color-gray-300)]">{detail()}</p>
        </div>
        <Show when={browserOnline() && reconnecting()}>
          <button
            type="button"
            class="shrink-0 rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-[11px] font-black text-white transition-colors hover:bg-white/15 focus-visible:ring-2 focus-visible:ring-[var(--custom-accent,#818cf8)] min-h-[44px]"
            onClick={reconnect}
          >
